...
namespace FedAllChampionsUtility
{
    ...
    {        
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("Combo").AddItem(new MenuItem("TypeCombo", "").SetValue(new StringList(new[] { "Mixed mode", "Burst combo", "Long combo" }, 0)));
            ...
            Program.Menu.SubMenu("Farm").AddItem(new MenuItem("FreezeActive", "Freeze!").SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
            ...
            Program.Menu.SubMenu("JungleFarm").AddItem(new MenuItem("JungActive", "Farm!").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
            ...
            Program.Menu.SubMenu("Extra").AddItem(new MenuItem("UsePacket", "Use Packet Cast").SetValue(true));
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Program.Menu.Item("HarassActive").GetValue<KeyBind>().Active) Harass();
            if (Program.Menu.Item("LaneClearActive").GetValue<KeyBind>().Active ||
                Program.Menu.Item("FreezeActive").GetValue<KeyBind>().Active) Farm();
            if (Program.Menu.Item("JungActive").GetValue<KeyBind>().Active) JungleFarm();
            if (Program.Menu.Item("UseSera").GetValue<bool>()) UseItems();
            if (Program.Menu.Item("KillSteal").GetValue<bool>()) KillSteal();
        }
        ...
        private static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            var UsePacket = Program.Menu.Item("UsePacket").GetValue<bool>();
        ...
        private static void Farm()
        {
            ...
            else if (Program.Menu.Item("LaneClearActive").GetValue<KeyBind>().Active)
            {
                foreach (var minion in allMinions)
                {
        ...
        private static void KillSteal()
        {
            ...
            if (KillSteal & (Q.IsReady() || W.IsReady() || E.IsReady()))
            {
                ...
                {
                    if (Q.IsReady() && IsKillable(ObjectManager.Player, enemy, new[] { SpellSlot.Q })) Q.CastOnUnit(enemy, UsePacket);
                    if (W.IsReady() && IsKillable(ObjectManager.Player, enemy, new[] { SpellSlot.W })) W.CastOnUnit(enemy, UsePacket);
                    if (E.IsReady() && IsKillable(ObjectManager.Player, enemy, new[] { SpellSlot.E })) E.CastOnUnit(enemy, UsePacket);
                }
